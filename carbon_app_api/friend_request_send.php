 ...
 *    - Duplicate pending requests are prevented (unique “pending pair”).
...
$me   = current_user_id();              // current authenticated user (requester)
$body = json_body();                    // safely parse JSON body -> assoc array
$to   = (int)($body['addressee_id'] ?? 0); // target user id (addressee)
...
try {
  ...
  // - Domain rules via triggers/constraints (e.g., can't friend yourself)
  $stmt = $pdo->prepare('INSERT INTO friend_requests (requester_id, addressee_id) VALUES (?, ?)');
  $stmt->execute([$me, $to]);

  // Fetch the newly created request’s status (usually "pending")
  ...
  // MySQL integrity constraint (duplicate, trigger-signal, etc.)
  if ($e->getCode() === '23000') {
    ...
    if ($ex = $q->fetch()) {
      if ($ex['status'] === 'pending') {
        // Tell client whether this is outgoing (I sent) or incoming (they sent)
  ...
  if (str_contains($msg, 'Cannot friend yourself'))  fail(400, 'Cannot friend yourself');     // self-guard (extra)
